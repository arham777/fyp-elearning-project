...
import { ChevronDown, Loader2, MessageSquare, Send, X } from "lucide-react";
...
const ChatWidget: React.FC = () => {
  ...
  if (!isOpen) {
    return (
      <Button
        ...
        className="fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-lg z-50 hover:scale-105 transition-transform duration-200 bg-[#7c3aed] hover:bg-[#6d28d9] text-white"
        size="icon"
      >
  ...
  return (
    <div className="fixed inset-0 z-50 pointer-events-none flex items-end justify-end p-4 sm:p-6" aria-label="Chatbot Container">
      <section 
        className="pointer-events-auto flex flex-col bg-background rounded-2xl shadow-2xl border border-border overflow-hidden transition-all duration-300 ease-in[max--out310px250px] h-[80vh] max-h-[calc(100vh-2rem)] sm:w-[380px] sm:h-[600px] sm:max-h-[600px]"
      >
      <header className="flex items-center justify-between px-4 py-4 bg-[#ff7800] text-white shrink-0">
        <div className="flex items-center gap-3">
           <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/20 backdrop-blur-sm">
              <MessageSquare className="h-5 w-5 text-white" />
           </div>
          <div>
            <h3 className="font-bold text-base leading-none tracking-tight">Your AI Assistant</h3>
                ...
                <div className="grid gap-2">
                    {[
                        ...
                        <button
                            ...
                            className="flex items-center gap-3 p-3 text-left bg-muted/30 hover:bg-[#7c3aed]/5 border border-transparent hover:border-[#7c3aed]/20 rounded-xl transition-all duration-200 group"
                        >
                            ...
                            <span className="text-sm font-medium text-foreground/80 group-hover:text-[#7c3aed]">{action.label}</span>
                            <Send className="w-3 h-3 ml-auto opacity-0 group-hover:opacity-100 text-[#7c3aed] -translate-x-2 group-hover:translate-x-0 transition-all" />
                        </button>
                    ))}
                </div>
             </div>
        ...
        {messages.map((message) => (
          <div key={message.id} className={`flex w-full ${message.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`flex flex-col max-w-[85%] ${message.role === "user" ? "items-end" : "items-start"}`}>
              <div
                className={`px-4 py-2.5 text-sm shadow-sm whitespace-pre-wrap leading-relaxed ${
                  message.role === "user"
                    ? "bg-[#7c3aed] text-white rounded-2xl rounded-tr-sm"
                    : "bg-muted border text-foreground rounded-2xl rounded-tl-sm"
                }`}
              >
                      ...
                      {message.reasoning.map((step, index) => (
                        <div key={`${message.id}-reason-${index}`} className="flex gap-2">
                           ...
                           <div>
                              ...
                              <div className="text-muted-foreground mt-0.5 leading-normal">{step.text}</div>
                           </div>
                        </div>
                      ))}
                    </div>
      ...
      <footer className="p-3 bg-background border-t">
        ...
        >
            ...
            <Button 
                ...
                className={`absolute right-1 top-1/2 -translate-y-1/2 h-8 w-8 rounded-full shadow-sm transition-all duration-200 ${
                    input.trim() ? "bg-[#7c3aed] text-white hover:bg-[#6d28d9]" : "bg-muted text-muted-foreground"
                }`}
            >
              {isSending ? (
